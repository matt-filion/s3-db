...
export class S3DBConfiguration {
   ...
   */
  public baseName: string = process.env.S3DB_ROOTNAME || 's3db'

  /**
   * Logical stage that the runtime is executing within.
   */
  public stage: string = process.env.S3DB_STAGE || 'dev'

  /**
   * Pattern used to create or look for the corresponding s3 bucket when
   * persisting an object.
   */
  public bucketPattern: string = process.env.S3DB_BUCKETPATTERN || '{{stage}}.{{region}}.{{baseName}}-{{bucketName}}'

  /**
   * Region to look for buckets.
   */
  public region: string = process.env.S3DB_REGION || process.env.AWS_DEFAULT_REGION || 'us-west-2'
}
